...
src/middleware.ts + src/lib/supabase/middleware.ts   session refresh + role-based routing only, not an authorization boundary — deliberately still `middleware.ts`, not Next 16's renamed `proxy.ts` (see Known Gotchas)
...
12. `0012_workout_logs_unique.sql` — unique constraint on `workout_logs (workout_exercise_id, trainee_id)` (dedup'd first, for a database that already has duplicates from before this migration). Lets `submitWorkout` (`src/app/trainee/actions.ts`) upsert instead of always inserting — a trainee toggling submit off/on, or resubmitting, used to insert a fresh duplicate log row every time, which is what the trainee history page (`src/app/trainee/history/page.tsx`) was showing as repeated entries.
...
- Next.js 16 renamed `middleware.ts` → `proxy.ts` (export `middleware` →
  `proxy`) — but **this repo deliberately stays on `middleware.ts`**, not
  the renamed `proxy.ts`. `AGENTS.md` (imported at the top of this file,
  auto-regenerated by `next dev` — keep it committed) has the pointer to
  read `node_modules/next/dist/docs/` before assuming anything about
  Next.js APIs matches training data; this version has real breaking
  changes, and this is one of them. See the next bullet for why.
- **`src/middleware.ts` is `middleware.ts`, not `proxy.ts`, on purpose —
  do not "modernize" this rename.** Next's own docs
  ...
  *"The edge runtime is NOT supported in proxy. The proxy runtime is
  ...
  edge runtime, keep using middleware."* `@opennextjs/cloudflare`'s own
  source (`dist/cli/build/open-next/bundle-node-middleware.js`) documents
  Node.js middleware on Cloudflare's `workerd` runtime as *"experimental
  ... not supported by the OpenNext maintainers."* Confirmed live
  2026-09-07: the project had used `proxy.ts` since its very first commit
  (verified via `git log --follow`), and the staging Worker
  (`yair-kanevsky-forge-staging`) was returning a generic 500 on **every** route — static and
  dynamic alike — with a real Cloudflare Observability log confirming the
  crash happened inside `middlewareHandler`, the exact code path the
  OpenNext comment above describes. Reverted back to `middleware.ts`,
  which routes through OpenNext's well-supported edge-middleware bundling
  instead. (Why did production, running the same file, appear unaffected
  at the time? Production is deployed by a separate, dashboard-configured
  Cloudflare "Workers Builds" git integration — see the ⚠️ warning under
  Deployment — whose build pipeline is not visible from this sandbox and
  was never confirmed to be building the same commit at the same time;
  this discrepancy was never fully resolved, only worked around by not
  depending on the unsupported code path at all.) The header-forwarding
...
- `src/middleware.ts` (proxy layer) uses `getSession()` (local cookie
  ...
  is. Don't "fix" this to `getUser()` for security reasons, it isn't a
  security gap — every real data query independently goes through RLS.
