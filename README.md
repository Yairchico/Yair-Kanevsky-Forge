...
6. **עדכון סכימה + ספריית תרגילים** (חד פעמי, אחרי המיגרציה הראשונית): ב-SQL Editor, הריצו בסדר הזה:
   ...
   10. `supabase/seed.sql` — מכניס ~58 תרגילי בסיס לספרייה (שמות באנגלית, קבוצת שריר בעברית). אפשר להריץ שוב בעתיד בלי סיכון לכפילויות (`ON CONFLICT DO NOTHING`). אם כבר הרצתם גרסה ישנה עם שמות עבריים — יש הערת ניקוי בראש הקובץ.
