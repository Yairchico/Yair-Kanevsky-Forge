...
6. **עדכון סכימה + ספריית תרגילים** (חד פעמי, אחרי המיגרציה הראשונית): ב-SQL Editor, הריצו בסדר הזה:
   ...
   9. `supabase/migrations/0010_soft_delete_programs.sql` — מחיקת תוכנית כבר לא מוחקת בפועל (`deleted_at` במקום `DELETE`) — כדי שהיסטוריית האימונים וההגשות של המתאמן לא תימחק יחד איתה.
   10. `supabase/migrations/0011_superadmin_readonly.sql` — מוסיף אפשרות ליצור חשבון "צפייה בלבד" (`profiles.is_superadmin`) — רואה הכל (כל מתאמן, כל תוכנית) אבל לא יכול לערוך/למחוק שום דבר. ראו "הגדרת חשבון צפייה בלבד (superadmin)" למטה.
   11. `supabase/migrations/0012_workout_logs_unique.sql` — מוסיפה אילוץ ייחודיות ל-`workout_logs` (תרגיל אחד לכל אימון שבועי) ומנקה כפילויות קיימות שנוצרו לפני התיקון — כדי שהגשה חוזרת של אימון תעדכן את הרשומה הקיימת במקום ליצור עוד אחת (זה מה שגרם לכפילויות בעמוד ההיסטוריה).
   12. `supabase/seed.sql` — מכניס ~58 תרגילי בסיס לספרייה (שמות באנגלית, קבוצת שריר בעברית). אפשר להריץ שוב בעתיד בלי סיכון לכפילויות (`ON CONFLICT DO NOTHING`). אם כבר הרצתם גרסה ישנה עם שמות עבריים — יש הערת ניקוי בראש הקובץ.
...
7. **הגדרת חשבון צפייה בלבד (superadmin), אופציונלי** — למישהו שצריך לראות הכל (כל מתאמן, כל תוכנית, כל היסטוריה) בלי יכולת לגעת בנתונים. **זה לא חשבון מאמן שני** — אין לו הרשאת כתיבה בכלל, בשום מקום, ברמת ה-DB (RLS), לא רק שהכפתורים מוסתרים ב-UI.
   1. Authentication → Users → Add user (בדיוק כמו יצירת המאמן בשלב 4, בלי `role: "trainer"` ב-metadata — ברירת המחדל `trainee` בסדר).
   2. ב-SQL Editor: `update public.profiles set is_superadmin = true where id = '<ה-UUID של המשתמש שיצרתם>';`
   3. התחברות עם החשבון הזה מובילה אוטומטית לאזור המאמן (`/trainer`) — אותם מסכים בדיוק, עם תג "צפייה בלבד" בכותרת ובלי אף כפתור עריכה/מחיקה/יצירה.
