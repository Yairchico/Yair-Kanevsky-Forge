...
- [x] שלב 0 – תשתית
- [x] שלב 1 – ליבת נתונים
- [x] שלב 2 – Builder (הוספה/עריכה/מחיקה/שכפול + Draft/Publish; שינוי סדר כרגע בכפתורי מעלה/מטה, לא drag&drop אמיתי — dnd-kit כבר מותקן, שדרוג עתידי קל)
- [x] שלב 3 – חוויית מתאמן (מסך שבועי, סימון כל תרגיל בנפרד + הגשת אימון שלם, טופס ביצוע בסיסי (משקל/חזרות/RPE בפועל), מסך היסטוריה ל-30 יום, המאמן רואה סטטוס לפי תרגיל דרך "השבוע שלו")
  - הערת מודל נתונים: `workout_completions` הוא flag יחיד per workout (לא per-שבוע-קלנדרי) — אז "הוגש/לא הוגש" משקף רק את המצב האחרון, לא היסטוריה של כמה שבועות. `workout_logs` כן צובר היסטוריה אמיתית (כל רשומה נשמרת), ולכן מסך ההיסטוריה מבוסס עליו. תמיכה מלאה בהיסטוריית "כמה פעמים עשית את היום הזה" תדרוש בעתיד מופעי תוכנית מתוארכים (program instances), לא רק program_days קבועים.
- [ ] שלב 4 – ליטוש ו-QA
