...
## רשימת המתנה — הערות להמשך (טרם בוצע, לא לבצע ביוזמה עצמית)

נרשם [2026-09-05] מבקשת המשתמש — "אלה הערות להמשך, לא נעשה אותן עכשיו". להתחיל מכאן בפעם הבאה שמבקשים להמשיך על הפריטים האלה:

1. **להציג באיזה יום בשבוע כל אימון** — כרגע אימון מוצג רק כ"אימון 1/2/3" (ממוספר, לא לפי יום). להוסיף איזשהו קישור ליום קלנדרי בפועl בתוך השבוע (למשל את המתאמן/המאמן לבחור/לראות איזה יום זה).
2. **הוספת תרגיל חדש לספרייה ישירות מתוך בורר התרגילים** (`add-exercise-picker.tsx`) כשהחיפוש לא מוצא תוצאה — כרגע היחיד דרך ליצור תרגיל היא `/trainer/exercises/new`, מסך נפרד; לשקול הוספה inline מתוך תיבת החיפוש עצמה כשאין תוצאות.
3. **אופציה מהירה למחיקת תוכנית** — `deleteProgram` קיים כ-Server Action (`programs/actions.ts`) אבל **לא מחובר לשום מקום ב-UI כרגע** — אין כרגע שום דרך למאמן למחוק תוכנית מהממשק. צריך כפתור/אופציה (כנראה בכרטיס התוכנית ברשימה בדף המתאמן, עם אישור).
4. **להנגיש יותר בקלות עריכת תוכנית קיימת** — כרגע דרך לעבור לבילדר היא ללחוץ על שורת התוכנית ברשימה בדף המתאמן; לבדוק אם צריך נתיב/כפתור ברור/מהיר יותר (למשל מה"פעילות אחרונה" בדף הבית, או מתוך "התוכנית השבועית").
5. **לעבור שוב על לוגיקת שכפול שבוע** (`programs/new/new-program-form.tsx` + `programs/actions.ts`) — התצוגה המקדימה הקיימת מוצגת *לפני* הבחירה (ברשימת השבועות המועמדים, לפני יצירת התוכנית). המשתמש רוצה שהתצוגה המקדימה תוצג *כחלק מתהליך השכפול עצמו* — כנראה כאישור/סקירה אחרונה של מה שעומד להישכפל, ממש לפני הביצוע בפועל (או אחרי היצירה, לפני שהתוכנית מסומנת כמוכנה) — לחדד את הזרימה המדויקת עם המשתמש כשמתחילים על זה.
