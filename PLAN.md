...
- **[2026-09-06] תוספת: `is_superadmin` (migration 0011)** — עדיין "מאמן יחיד", לא multi-tenant. זו לא רשומת `role='trainer'` שנייה (שהייתה נותנת גישת כתיבה מלאה) אלא flag בוליאני נפרד על `profiles`, שמעניק **קריאה בלבד** על כל הטבלאות (בלי שום מדיניות INSERT/UPDATE/DELETE) — למשתמש נוסף שרוצה "לצפות בהכל" (כל המתאמנים, כל התוכניות) בלי יכולת לגעת בנתונים של המאמן. נוצר ידנית ב-Supabase, בדיוק כמו המאמן היחיד היום.
...
- [~] שלב 4 – ליטוש ו-QA (בתהליך)
  ...
  - [ ] **[2026-09-06] תגלית תשתית קריטית, לא פתורה**: הענף `claude/fitness-app-trainers-gccazz` מחובר ב-Cloudflare עצמה ("Workers Builds") ישירות ל-Worker של **production** — כל push אליו יכול לעלות לאתר האמיתי מיד, במקביל ומבלי קשר ל-`deploy.yml`/`deploy-production.yml` שלנו (שנשארו ב-0 הרצות ל-production לאורך כל הזמן — בלתי נראה מצד GitHub לגמרי). התגלה כשהמשתמש ראה שינויים טריים בקישור ה-production. **נוסה תיקון**: מעבר לענף פיתוח נפרד, `testing` — **לא עבד**: אושר בדשבורד Cloudflare (טאב Deployments, build מתויג `testing`) שה-git integration מופעל על push לכל ענף, לא רק לענף המקורי. **החלטת המשתמש [2026-09-06]**: "תדחוף למיין וזהו, נטפל בזה אחר כך" — לחזור לפתח ולדחוף ל-`claude/fitness-app-trainers-gccazz` כרגיל ולקבל את הסיכון הזה כידוע ומקובל, במקום לחסום עבודה שוטפת עליו. `deploy.yml` הוחזר לטריגר על הענף הזה. **התיקון האמיתי — ניתוק ה-git integration בדשבורד Cloudflare (Workers & Pages → `yair-kanevsky-forge` → Settings → Build) — עדיין לא בוצע**, באחריות המשתמש. ראו CLAUDE.md's "Deployment" section לפרטים המלאים.

---

## רשימת המתנה — הערות להמשך

נרשם [2026-09-05] מבקשת המשתמש כהערות להמשך; בוצעו בפועל ב-[2026-09-06] לאחר הנחיה מפורשת להתחיל ליישם. סטטוס:

1. [x] **יום בשבוע לכל אימון** — שבוע מוגדר ראשון–שבת במערכת (migration `0009_workout_days.sql`, `workouts.day_of_week`), עד 2 אימונים ליום (נאכף גם ב-DB trigger), המאמן בוחר יום לפני יצירת אימון (day-picker `Modal`). מספור "אימון N" גלובלי בוטל — נגזר בתצוגה מ-`(day_of_week, order_index)`.
2. [x] **הוספת תרגיל חדש לספרייה ישירות מתוך בורר התרגילים** — כשהחיפוש לא מוצא תוצאה, כפתור "+ הוספת תרגיל" פותח טופס inline בתוך `add-exercise-picker.tsx` (שם/קבוצת שריר/ציוד/הנחיות/תמונה), מוסיף ישירות לאימון הנוכחי (`createExerciseInline`, `src/app/trainer/exercises/actions.ts`). בנוסף: תרגיל חדש נוצר **ללא תמונה** כברירת מחדל (`media_url: null`) — לא יותר תמונת דמה/ניחוש; ניתן למחוק תרגיל מהספרייה (`deleteExercise`, מטפל בשגיאת FK כשתרגיל בשימוש).
3. [x] **אופציה מהירה למחיקת תוכנית** — `ProgramRow` (`trainees/[id]/program-row.tsx`) מציג כפתור מחיקה עם אישור inline ליד כל תוכנית ברשימה.
4. [x] **גישה מהירה יותר לעריכת תוכנית קיימת** — כל שורת תוכנית היא עצמה קישור לבילדר (מסומנת ב-`Pencil`), לא רק טקסט.
5. [x] **תצוגה מקדימה לפני שכפול שבוע** — חלון popup במרכז המסך (`Modal`) מציג אימון אחד בכל פעם (יום + רשימת תרגילים) עם חיצי ניווט (`ChevronRight`/`ChevronLeft`) בין אימוני השבוע המועמד; רק אישור בתוך המודאל בפועל בוחר את השבוע, ביטול/סגירה משאיר את הבחירה כפי שהייתה.
6. [x] **תצוגת מאמן: משקלים מתוכננים מול בפועל** — כרטיס תרגיל בתצוגת השבוע של המאמן (`trainee-week-view.tsx`) מציג תמיד "מתוכנן: ..." ומתחתיו (אם הוגש) "בפועל (בהגשה): ...".
7. [x] **מסך מתאמן: הזנת ביצוע ללא שמירה פר-תרגיל** — שדות "בפועל" (משקל/חזרות/RPE/הערה) גלויים תמיד תחת כל תרגיל, ללא כפתור שמירה נפרד; הערכים נשמרים כטיוטה מקומית (`src/lib/workout-draft.ts`, `localStorage` לפי `workoutId`) ושורדים סגירה/פתיחה מחדש של הדפדפן. לחיצה על "הגש אימון" היא הרגע היחיד שבו נכתבת שורת `workout_logs` לכל תרגיל שיש בו תוכן, בבת אחת (`submitWorkout` ב-`src/app/trainee/actions.ts`).
