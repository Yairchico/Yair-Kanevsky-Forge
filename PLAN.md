...
- [~] שלב 4 – ליטוש ו-QA (בתהליך)
  - [x] בדיקת RLS סטטית (מעבר על כל הפוליסות מול כל טבלה): אין דליפת מידע בין מתאמנים, טיוטה לא נגישה למתאמן, הגנה מפני self-escalation של role. שתי הערות זניחות שנרשמו לתיעוד: (1) `email_for_username` חושף (דרך קריאה ישירה ל-RPC, לא דרך ה-UI) האם שם משתמש קיים; (2) מתאמן יכול טכנית לערוך/למחוק `workout_logs`/`workout_completions` שלו-עצמו אחרי ההזנה (RLS "for all" ולא רק insert) — לא בעיית בידוד בין מתאמנים, סיכון נמוך.
