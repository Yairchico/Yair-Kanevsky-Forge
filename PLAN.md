...
7. [ ] **מסך מתאמן: הזנת ביצוע ללא שמירה פר-תרגיל** — עדיין לא בוצע (המשימה הבאה בתור): לבטל את כפתור השמירה הבודד לכל תרגיל, לשמור טיוטה מקומית (כולל שרידות אחרי סגירת/פתיחת דפדפן מחדש דרך `localStorage`), ולשלוח הכל יחד רק בלחיצה על "הגש אימון".
